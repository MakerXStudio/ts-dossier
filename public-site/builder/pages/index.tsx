...
import { getPageMetaData } from '../shared/build-time/pages'
...
const IndexPage = (props: IIndexProps) => {
    ...
    //         <section>
    //             <h2>Posts</h2>
    //             {sortedPages.map((pageMeta) => (
    //                 <article key={pageMeta.slug}>
    //                     <Link href={`/${pageMeta.slug}`}>
    //                         <a>{pageMeta.title}</a>
    //                     </Link>
    //                 </article>
    //             ))}
...
export const getStaticProps: GetStaticProps = async (): Promise<{ props: IIndexProps }> => {
    const blogs = await getPageMetaData()
    return {
        props: { pages: blogs },
    }
}
